...
def plot_category_performance_by_location():
    ...
    REQUIRED: Return the figure object
    """
    recent_quarter = quarter_labels[-1]
    category_perf = sales_df[sales_df['QuarterLabel'] == recent_quarter].groupby(['Location', 'Category'])['Sales'].sum().unstack().reindex(locations)
    fig, ax = plt.subplots(figsize=(10, 5))
    x = np.arange(len(locations))
    width = 0.15
    for i, category in enumerate(categories):
        ax.bar(x + i * width, category_perf[category], width, label=category)
...
def plot_sales_composition_by_location():
    ...
    REQUIRED: Return the figure object
    """
    composition = sales_df.groupby(['Location', 'Category'])['Sales'].sum().unstack().reindex(locations)
    composition_pct = composition.div(composition.sum(axis=1), axis=0) * 100
    fig, ax = plt.subplots(figsize=(10, 5))
    bottom = np.zeros(len(locations))
    for category in categories:
        ax.bar(locations, composition_pct[category], bottom=bottom, label=category)
        bottom += composition_pct[category].values
...
def plot_ad_spend_vs_sales():
    ...
    x = np.linspace(sales_df['AdSpend'].min(), sales_df['AdSpend'].max(), 100)
    ax.plot(x, m * x + b)
    outlier = sales_df.loc[sales_df['Sales'].idxmax()]
    ax.annotate(f"{outlier['Location']} - {outlier['Category']}", (outlier['AdSpend'], outlier['Sales']))
...
def plot_ad_efficiency_over_time():
    ...
    ax.annotate(f"{efficiency.iloc[-1]:.2f}", (efficiency.index[-1], efficiency.iloc[-1]))
...
def plot_customer_age_distribution():
    ...
    axes = axes.flatten()
    groups = [('Overall', customer_df)] + [(location, customer_df[customer_df['Location'] == location]) for location in locations]
    for i, (name, data) in enumerate(groups):
        axes[i].hist(data['Age'], bins=15)
        axes[i].axvline(data['Age'].mean(), linestyle='--', label='Mean')
        axes[i].axvline(data['Age'].median(), linestyle=':', label='Median')
...
def plot_purchase_by_age_group():
    ...
    REQUIRED: Return the figure object
    """
    age_groups = pd.cut(customer_df['Age'], bins=[18, 31, 46, 61, 81], right=False, labels=['18-30', '31-45', '46-60', '61+'])
    plot_df = customer_df.copy()
    plot_df['AgeGroup'] = age_groups
    fig, ax = plt.subplots(figsize=(10, 5))
    plot_df.boxplot(column='PurchaseAmount', by='AgeGroup', ax=ax)
...
def plot_sales_by_price_tier():
    ...
    explode = [0.1 if v == tier_sales.max() else 0 for v in tier_sales]
    fig, ax = plt.subplots(figsize=(8, 8))
    ax.pie(tier_sales.values, labels=tier_sales.index, autopct='%1.1f%%', explode=explode)
...
def plot_category_market_share():
    ...
    fig, ax = plt.subplots(figsize=(8, 8))
    ax.pie(category_sales.values, labels=category_sales.index, autopct='%1.1f%%', explode=explode)
...
def plot_location_sales_distribution():
    ...
    fig, ax = plt.subplots(figsize=(8, 8))
    ax.pie(location_sales.values, labels=location_sales.index, autopct='%1.1f%%')
...
def main():
    ...
    print(f"- Highest total sales quarter: {sales_df.groupby('QuarterLabel')['Sales'].sum().idxmax()}")
    print(f"- Top performing location: {sales_df.groupby('Location')['Sales'].sum().idxmax()}")
    print(f"- Largest product category: {sales_df.groupby('Category')['Sales'].sum().idxmax()}")
    print(f"- Largest price tier by purchase amount: {customer_df.groupby('PriceTier')['PurchaseAmount'].sum().idxmax()}")
